...
function clearPreviousSearch() {
    ...
    document.getElementById('currentImg').textContent = ""
...
function fetchAPI() {
    ...
    fetch(latLonUrl)
        ...
        .then(function (geocodeData) {
            ...
            // if i got here then the city i typed is most likely the city i wanted
          
            // take the city that was searched
            const button = document.createElement('button')
            $(button).addClass('btn btn-primary m-2');
            button.innerHTML = citySearch
            if(document.getElementById('city-search').value === $(button).value){
                return
            }
            else{
            // put in on the screen in the designated area
           document.querySelector('#previous-searches').appendChild(button)
            // when button is clicked value is searched
            button.addEventListener('click', function(){
                // get the city that was clicked (innerhtml of the button)
                var theCity = citySearch;
                // reach into the html and get the input box
                var inputBox = document.querySelector('#city-search')
                // put it in the search boxs value
            ...
            let citiesFromLS = localStorage.getItem('searchedCities');
            let citiesAsJS = JSON.parse(citiesJSON);
            let newCity = cities;
            //citiesAsJS.push(newCity);
            console.log(citiesAsJS);
...
//when search is clicked, if fetch API is successful 